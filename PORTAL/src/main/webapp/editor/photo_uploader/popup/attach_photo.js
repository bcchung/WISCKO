	...
	function checkDragAndDropAPI(){
		//dragAndDrop 필요성 못느낌..파일 추가해서 첨부하도록 bSupportDragAndDropAPI = false; 로 강제 지정.
		/*try{
			if( !oNavigator.ie ){
    ...
    function html5Upload() {	
    	...
    	sUploadURL= 'http://'+window.location.host+'/PORTAL/editor/ImageUpload.jsp'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : 'http://'+window.location.host+'/PORTAL/editor/ImageUpload.jsp',
